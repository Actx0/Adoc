...
The MCP server is a thin proxy: every tool call uses your workspace access key against the Actx0 API. Scope is fixed per connection — workspace, agent, and session come from headers, not from tool arguments.
...
2. Create an [agent](/actx0-platform/agent) in the workspace.
