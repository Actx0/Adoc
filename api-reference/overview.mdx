...
Agent, session, message, memory, prompt, and knowledge endpoints include package samples for **pctx0** (Python), **nctx0** (Node.js), **gtcx0** (Go), and **acli** (CLI), alongside curl.
...
These clients authenticate with a workspace access key (`X-Access-Key`). API key, workspace, and access-key management endpoints are REST/curl only.
...
Self-hosted or local deployments use your own host with the same `/api/v1` prefix (for example `http://127.0.0.1:8000/api/v1`).
...
<CardGroup cols={2}>
  ...
  <Card title="Access Keys" icon="key-round" href="/api-reference/create-access-key">
